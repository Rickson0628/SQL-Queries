/*****************************************************************************************************************************
** ------- First Student: Rickson Pantaleon Bozar
** Student ID: 167549237
** Student Full Name: Rickson Pantaleon Bozar
** ------- Second Student
** Student ID: 167550235
** Student Full Name: John Richard Pantaleon Bozar
** ------- Third Student
** Student ID:
** Student Full Name:
...
int main(void)
{
    ...
    /* Used Variables */
    string str;
    string user = "dbs311_243ncc09";
    string pass = "30438808";
...
void displayOrderStatus(Connection* conn, int orderId, int customerId) {
    ...
    string status;
    try {
        // Prepare the SQL statement to retrieve order status
        stmt = conn->createStatement("BEGIN get_order_status(:1, :2, :3); END;");
        stmt->setInt(1, orderId);
        stmt->setInt(2, customerId);
        stmt->registerOutParam(3, Type::OCCISTRING, 100); // Adjust size as needed

        // Execute the statement
        stmt->executeUpdate();

        // Retrieve the output parameter (order status)
        ...
        cout << "Error while fetching order status." << endl;
...
void cancelOrder(Connection* conn, int orderId, int customerId) {
    ...
    try {
        ...
        cout << "DEBUG: orderId = " << orderId << ", customerId = " << customerId << endl;

        // Prepare the SQL statement
        stmt = conn->createStatement("BEGIN cancel_order(:1, :2, :3); END;");
        stmt->setInt(1, orderId);
        stmt->setInt(2, customerId);
        stmt->registerOutParam(3, Type::OCCIINT);

        // Execute the statement
        cout << "Executing cancel_order procedure..." << endl;
        stmt->executeUpdate();
        cout << "Execution completed." << endl;

        // Retrieve the output parameter
        int result = stmt->getInt(3);
        cout << "DEBUG: cancelStatus = " << result << endl;

        // Check the result
        if (result == 1) {
            cout << "Order has been successfully cancelled." << endl;
        }
        else if (result == 2) {
            cout << "Order cannot be canceled because it is shipped." << endl;
        }
        else if (result == 0) {
            cout << "Order does not exist." << endl;
        }
        else {
            cout << "Order cancellation failed. Please check the order ID and customer ID." << endl;
        }
        ...
        cout << "Error while cancelling order." << endl;
        cout << "Error Code: " << sqlExcp.getErrorCode() << endl;
        cout << "Message: " << sqlExcp.getMessage() << endl;
        if (stmt != nullptr) {
            conn->terminateStatement(stmt);
        }
    }
